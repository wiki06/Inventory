...
public class PosController implements Initializable, ProductInterface {
    ...
    @FXML
    private TableColumn<Item, Double> priceColumn, quantityColumn, totalColumn, cgstColumn, sgstColumn, cgstperColumn, sgstperColumn;
    @FXML
    ...
    private void showDetails(Product product) {
    	System.out.println("called while clicking table");
        if (product != null) {
    ...
    public void addAction(ActionEvent event) {
        ...
        if (validateInput()) {
            ...
            double cgstper= Double.valueOf(cgstField.getText());
            double sgstper= Double.valueOf(sgstField.getText());
            System.out.println("cgst percentage--->"+cgstper+"   "+sgstper);
            double cgst = (Double.valueOf(cgstField.getText()) / 100) * subtotal;
    ...
    private void calculation() {
        ...
        subTotalPrice = totalsum;
        if (subTotalPrice > 0) {
            paymentButton.setDisable(false);
            System.out.println("get the actually value---->"+cgstField.getText());
            /*double cgst = (double) subTotalPrice *  (Double.parseDouble(cgstField.getText()) / 100);
            double sgst = (double) subTotalPrice *  (Double.parseDouble(sgstField.getText()) / 100);*/
            double netPayablePrice = subTotalPrice;
